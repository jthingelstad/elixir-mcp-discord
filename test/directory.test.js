...
test("inherited permission is not enough; an explicit grant or a binding is", () => {
  ...
  assert.equal(
    classify({ ...base, explicitSend: true, canSend: false }),
    null,
    "a grant the overwrites then deny is no grant",
  );
});
...
test("an explicit grant is an overwrite for the bot's role or the bot itself allowing Send", () => {
  ...
  assert.equal(
    explicitGrant([{ id: "bot", type: 1, allow: str(P.SendMessages | P.ViewChannel), deny: "0" }], ids),
    true,
  );
  assert.equal(
    explicitGrant([{ id: "bot", type: 1, allow: str(P.ViewChannel), deny: "0" }], ids),
    false,
    "View alone is not a grant to post",
  );
});
...
test("fromRest builds the directory the boot check and setup agree on", () => {
  const inspected = {
    ...
    channels: [
      { id: "10", name: "general", topic: "Everyone", type: 0, position: 0, guild_id: "g", permission_overwrites: [] },
      {
        ...
        position: 1,
        guild_id: "g",
        permission_overwrites: [{ id: "botrole", type: 0, allow: str(P.SendMessages), deny: "0" }],
      },
      {
        id: "12",
        name: "leaders",
        ...
        permission_overwrites: [
          {
            id: "botrole",
            type: 0,
            allow: str(P.SendMessages | P.CreatePublicThreads | P.SendMessagesInThreads),
            deny: "0",
          },
        ],
      },
  ...
  assert.deepEqual(
    ...
    ["news", "leaders", "ask"],
    "#general is inherited and left out",
  );
