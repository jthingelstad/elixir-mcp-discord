...
const timeline = [
  { at: "2026-09-13T21:00:00Z", subject_tag: "#A", subject_name: "sikander", kind: "battle_session", section: "activity", text: "sikander played 12 battles", facts: { battles: 12 } },
  { at: "2026-09-13T21:05:00Z", subject_tag: "#B", subject_name: "New", kind: "member_joined", section: "roster", text: "New joined", facts: {} },
  { at: "2026-09-13T21:06:00Z", subject_tag: "#C", subject_name: "Ollie", kind: "ranked_promotion", section: "standouts", text: "Ollie reached Champion", facts: { league: "Champion" } },
  { at: "2026-09-13T21:07:00Z", subject_tag: "#D", subject_name: "hamster", kind: "returned", section: "presence", text: "hamster is back after 7 days", facts: { after_days: 7 } },
];

test("a routine wakes for its kinds, or its sections, or everything if it names neither", () => {
  assert.deepEqual(relevant(timeline, { kinds: ["member_joined", "member_left"] }).map((i) => i.kind), ["member_joined"]);
  assert.deepEqual(relevant(timeline, { sections: ["roster", "presence"] }).map((i) => i.kind), ["member_joined", "returned"]);
