...
const turn = (overrides = {}) => {
  ...
  return ledger.turnEntry({
      ...
      trace: [
        {
          ...
          input: {},
          shape: "1 players",
          result: JSON.stringify({ wins: 9, losses: 2 }),
        },
      ],
      envelopes: [],
      usd: 0.01,
    ...
    input: {
      kind: "message",
      asker: { id: "42", name: "Tester" },
      channelId: "2",
      threadId: null,
      question: overrides.question ?? "how am I playing?",
      history: [],
    },
    output: {
      text: overrides.text ?? "9-2 today.",
...
test("a memory entry appends as one dated line with its provenance; other files append raw text", () => {
  const ok = planEdit({
    file: "memory.md",
    edit: { op: "append", text: "- 2026-09-14 (turns a1b2c3d4): pass segment to battles_meta_cards" },
    current: "# Memory\n",
  });
  ...
  assert.equal(
    planEdit({ file: "memory.md", edit: { op: "append", text: "pass segment" }, current: "" }).ok,
    false,
    "undated is refused",
  );
  assert.equal(
    planEdit({ file: "memory.md", edit: { op: "append", text: "- 2026-09-14: no provenance" }, current: "" }).ok,
    false,
  ...
  assert.equal(
    planEdit({
      file: "memory.md",
      edit: { op: "append", text: "- 2026-09-14 (from owner) until 2026-09-21: pushing for top 10" },
      current: "",
    }).ok,
    true,
    "owner entries with an expiry",
  );
  ...
  assert.match(
    planEdit({
      file: "memory.md",
      edit: { op: "remove", find: "- 2026-09-14 (from owner): we call war days boat days" },
      current: owned,
    }).error,
  ...
  assert.equal(
    planEdit({
      file: "memory.md",
      edit: { op: "remove", find: "- 2026-09-14 (from owner): we call war days boat days", by: "owner" },
      current: owned,
    }).ok,
    true,
  );
  const rule = planEdit({
    file: "identity.md",
    edit: { op: "append", text: "- Never names a member's losses in a channel the clan reads." },
    current: "## What it never does\n\n- Never guesses.\n",
  });
...
test("replace needs the text to occur exactly once, and never inside a routine's front matter", () => {
  ...
  assert.equal(
    planEdit({
      ...
      edit: { op: "replace", find: "at: 01:00", replace: "at: 02:00" },
      current: routine,
    }).ok,
    false,
    "front matter",
  ...
  const ok = planEdit({
    file: "routines/movers.md",
    edit: { op: "replace", find: "Name three movers.", replace: "Name at most three movers." },
    current: routine,
  });
  assert.equal(ok.ok, true);
  ...
  assert.equal(
    planEdit({ file: "identity.md", edit: { op: "replace", find: "e", replace: "x" }, current: "eee" }).ok,
    false,
    "ambiguous",
  );
  assert.equal(
    planEdit({ file: "identity.md", edit: { op: "replace", find: "zzz", replace: "x" }, current: "abc" }).ok,
    false,
    "absent",
  );
});
...
test("memory.md is bounded by entries and by characters", () => {
  ...
  assert.match(
    planEdit({ file: "memory.md", edit: { op: "append", text: "- 2026-09-14 (turns y): one more" }, current: twenty })
      .error,
    /already has 20/,
  );
  const fat = `- 2026-09-01 (turns x): ${"x".repeat(5990)}`;
  assert.match(
    planEdit({ file: "memory.md", edit: { op: "append", text: "- 2026-09-14 (turns y): tip" }, current: fat }).error,
    /exceed/,
  );
});
...
test("flagged turns come first and in full; plain turns are compact", () => {
  ...
  const flagged = {
    ...turn({ turnId: "bbbb0002" }),
    reactions: [{ reaction: "down", note: "wrong week", userId: "7" }],
    filed: [],
    findings: [],
    interventions: [{ by: "other_member", text: "try players_search", userId: "8" }],
  };
    ...
    askFn: fakeAsk([
      [
        "propose_change",
        {
          ...
          rule: "windows",
          turn_ids: ["bbbb0002"],
          summary: "Say which window a summary covers.",
          edit: { op: "append", text: "- 2026-09-14 (turns bbbb0002): name the window in every summary" },
        },
      ],
      [
        "propose_change",
        {
          file: "identity.md",
          rule: "voice",
          turn_ids: ["aaaa0001", "bbbb0002"],
          summary: "Shorter.",
          edit: { op: "replace", find: "Plain and direct.", replace: "Plain, direct, short." },
        },
      ],
      [
        "propose_change",
        {
          ...
          turn_ids: ["aaaa0001"],
          summary: "no",
          edit: { op: "append", text: "- 2026-09-14 x" },
        },
      ],
      [
        "report_mechanics",
        { rule: "WHO_IS_ASKING", turn_ids: ["bbbb0002"], summary: "asked for a tag it could look up" },
      ],
    ]),
  });
  ...
  const inner = fakeAsk([
    [
      "propose_change",
      {
        ...
        turn_ids: ["aaaa0001"],
        summary: "s",
        edit: { op: "append", text: "- 2026-09-14 (turns aaaa0001): x" },
      },
    ],
  ]);
  const outcome = await runReview({
...
test("the proposal cap holds, and a refused proposal says why", async (t) => {
  ...
  const calls = Array.from({ length: 5 }, (_, i) => [
    "propose_change",
    {
      ...
      turn_ids: ["aaaa0001"],
      summary: `s${i}`,
      edit: { op: "append", text: `- 2026-09-14 (turns aaaa0001): lesson ${i}` },
    },
  ]);
  ...
  const outcome = await runReview({
    trigger: "command",
    agentDir: dir,
    askFn: async (args) => (seen = await askFn(args)),
  });
...
test("the DM for a proposal carries the diff and the right buttons for its state", () => {
  ...
  const proposal = {
    ...
    rule: "windows",
    turnIds: ["a", "b"],
    summary: "Name the window.",
    preview: "+ - 2026-09-14 (turns a, b): name the window",
  };
  ...
  assert.deepEqual(
    pending.buttons.map((b) => b.label),
    ["Apply", "Try it", "Skip", "Show turns"],
  );
  const applied = proposalMessage(review, proposal, {
    index: 1,
    total: 2,
    decision: { decision: "applied", by: "9", detail: { backup: "/x/.history/memory.md.t" } },
  });
  assert.match(applied.content, /✅ Applied by <@9>/);
  assert.deepEqual(
    applied.buttons.map((b) => b.label),
    ["Undo", "Show turns"],
  );
    ...
    askFn: fakeAsk([
      [
        "propose_change",
        {
          ...
          turn_ids: ["aaaa0001"],
          summary: "s",
          edit: { op: "append", text: "- 2026-09-14 (turns aaaa0001): a lesson" },
        },
      ],
    ]),
  });
...
test("a sweep verdict parses into a class, and only ELIXIR files", () => {
  assert.deepEqual(parseVerdict("PROMPT: the brief should name the window"), {
    cls: "prompt",
    note: "the brief should name the window",
  });
  assert.deepEqual(parseVerdict("MECHANICS: identity rule asks for a tag it could look up"), {
    cls: "mechanics",
    note: "identity rule asks for a tag it could look up",
  });
...
function threadMessage({ authorId, starterAuthorId, content, priorBotMessageId }) {
  ...
  const prior = new Map([
    [priorBotMessageId, { id: priorBotMessageId, author: { bot: true }, cleanContent: "9-2 today.", pinned: false }],
  ]);
  return {
    id: "q2",
    ...
    channel: {
      ...
      fetchStarterMessage: async () => ({
        id: "q1",
        author: { id: starterAuthorId, bot: false },
...
test("another member speaking in the thread is recorded as an intervention on the bot's last turn", async () => {
  ...
  await handleAsk(
    threadMessage({
      authorId: "leader",
      starterAuthorId: "asker",
      content: "try players_search for their name",
      priorBotMessageId: "bot-msg-1",
    }),
    routine,
    { askFn: async () => result },
  );
...
test("a 👎 the sweep pins on this bot becomes a finding, not a filing", async () => {
  ...
  const out = await handleReaction(
    { emoji: { name: "👎" }, message },
      ...
      sweepFn: async ({ turn }) => {
        ledger.append(
          ledger.findingEntry({
            ...
            cls: "prompt",
            source: "reaction",
            note: "the brief never names the window",
          }),
        );
...
test("set_fields rewrites the front matter, is checked by the routine parser, and is the operator's alone", () => {
  const ok = planEdit({
    file: "routines/movers.md",
    edit: { op: "set_fields", by: "owner", fields: { at: "07:30", days: "mon,thu", enabled: "false" } },
    current: MOVERS,
  });
  assert.equal(ok.ok, true);
  assert.match(
  ...
  assert.match(
    planEdit({
      file: "routines/movers.md",
      edit: { op: "set_fields", by: "owner", fields: { at: "25:00" } },
      current: MOVERS,
    }).error,
    /not a real time/,
  );
  assert.match(
    planEdit({
      file: "routines/movers.md",
      edit: { op: "set_fields", by: "owner", fields: { catchup: "2" } },
      current: MOVERS,
    }).error,
    /not a routine field/,
  );
  assert.match(
    planEdit({
      file: "routines/movers.md",
      edit: { op: "set_fields", by: "owner", fields: { at: "" } },
      current: MOVERS,
    }).error,
    /at must be HH:MM/,
    "removing a required field is refused by the parser",
  );
  assert.match(
    planEdit({ file: "routines/movers.md", edit: { op: "set_fields", fields: { at: "07:30" } }, current: MOVERS })
      .error,
    /operator's/,
    "the review cannot reschedule",
  );
  assert.match(
    planEdit({ file: "memory.md", edit: { op: "set_fields", by: "owner", fields: { at: "07:30" } }, current: "" })
      .error,
    /for routines/,
  );
});
...
test("create makes a routine that parses, and refuses one that would not", () => {
  const ok = planEdit({
    ...
    edit: {
      op: "create",
      by: "owner",
      fields: { trigger: "schedule", at: "20:00", days: "fri", channel: "war", may_skip: "true" },
  ...
  assert.match(
    planEdit({
      file: "routines/war-recap.md",
      edit: { op: "create", by: "owner", fields: { trigger: "schedule" }, text: "x" },
      current: null,
    }).error,
    /at must be HH:MM/,
  );
  assert.match(
    planEdit({
      file: "routines/movers.md",
      edit: { op: "create", by: "owner", fields: { trigger: "schedule", at: "01:00" }, text: "x" },
      current: MOVERS,
    }).error,
    /already exists/,
  );
...
test("apply of a create writes the file and seeds its period; undo removes it; delete keeps a backup and undo restores", async (t) => {
  ...
  assert.ok(
    state.get("runs")?.["war-recap"],
    "the new routine's current period is marked done, so it does not fire on save",
  );
...
test("a setting is checked like setup checks it, previewed as a diff, and only the operator may change one", () => {
  ...
  const ok = planEdit({
    ...
    edit: {
      op: "set_config",
      by: "owner",
      fields: { ASK_MONTHLY_BUDGET_USD: "15", TIMEZONE: "America/Chicago", MONTHLY_BUDGET_USD: "25.00" },
    },
    current: cfg,
    by: "9",
  });
  ...
  assert.deepEqual(JSON.parse(ok.next), {
  ...
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { ELIXIR_MCP_URL: "x" } },
      current: cfg,
      by: "9",
    }).error,
    /not a setting the DM may change/,
    "wiring lives in .env and is not a setting",
  );
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { ANTHROPIC_API_KEY: "x" } },
      current: cfg,
      by: "9",
    }).error,
    /not a setting/,
  ...
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { REVIEW_AT: "sometime" } },
      current: cfg,
      by: "9",
    }).error,
    /REVIEW_AT/,
  ...
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { CLAUDE_MODEL: "claude-9" } },
      current: cfg,
      by: "9",
    }).error,
    /price|priced|claude-9/i,
  );
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { TIMEZONE: "CST" } },
      current: cfg,
      by: "9",
    }).error,
    /IANA/,
  ...
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { ADMIN_USER_IDS: "12345678" } },
      current: cfg,
      by: "9",
    }).error,
  ...
  assert.match(
    planEdit({ file: "config.json", edit: { op: "set_config", fields: { MONTHLY_BUDGET_USD: "1" } }, current: cfg })
      .error,
    /operator's/,
    "the review cannot touch settings",
  );
  assert.match(
    planEdit({
      file: "config.json",
      edit: { op: "set_config", by: "owner", fields: { MONTHLY_BUDGET_USD: "20.00" } },
      current: cfg,
      by: "9",
    }).error,
    /nothing would change/,
  );
  const unset = planEdit({
    file: "config.json",
    edit: { op: "set_config", by: "owner", fields: { MONTHLY_BUDGET_USD: "" } },
    current: cfg,
    by: "9",
  });
...
test("Try it rehearses a routine proposal on the proposed file, posts nothing, and charges the review lane", async (t) => {
    ...
    askFn: fakeAsk([
      [
        "propose_change",
        {
          ...
          rule: "brief",
          turn_ids: ["aaaa0001"],
          summary: "At most three.",
          edit: {
            op: "replace",
            find: "Name three movers.",
            replace: "Name at most three movers, fewer on a quiet day.",
          },
        },
      ],
      [
        "propose_change",
        {
          ...
          turn_ids: ["aaaa0001"],
          summary: "Shorter.",
          edit: { op: "replace", find: "Plain.", replace: "Plain and short." },
        },
      ],
    ]),
  });
  ...
  const runFn = async (routine, opts) => {
    ...
    return {
      ...
      skipped: false,
      text: "",
      posts: [{ channel: "#news", text: "**canavar** — 9-2." }],
      result: {
        usd: 0.03,
        trace: [],
        called: [],
  ...
  assert.equal(
    seen[1].overrides.identity,
    "Plain and short.",
    "identity.md proposals run as an override on a scheduled routine",
  );
});
